...
- **`run-fanout`** — N members that must run **isolated and in parallel**. Spawn one subagent per member (never inline — inline lets their contexts contaminate each other and defeats the fan-out). Each subagent uses the shared `instructions`, carries out its
